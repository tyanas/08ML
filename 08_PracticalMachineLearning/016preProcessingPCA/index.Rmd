...
```{r corrPredictors,dependson="loadPackage",cache=TRUE,fig.height=3.5,fig.width=3.5}
names(spam)[c(34,32)]
...
```{r rotatedPlot,dependson="loadPackage",cache=TRUE,fig.height=3.5,fig.width=3.5}
...
```{r spamPC-with-caret,dependson="spamPC",cache=TRUE,fig.height=3.5,fig.width=3.5}
...
# TODO modelFit <- train(training$type ~ .,method="glm",data=trainPC)
```
...
# TODO confusionMatrix(testing$type,predict(modelFit,testPC))
```
...
```{r ,dependson="pcaCaret",cache=TRUE,fig.height=3.5,fig.width=3.5}
# TODO modelFit <- train(training$type ~ .,method="glm",preProcess="pca",data=training)
# TODO confusionMatrix(testing$type,predict(modelFit,testing))
#Ошибка в predict(modelFit, testing) :объект 'modelFit' не найден
```
